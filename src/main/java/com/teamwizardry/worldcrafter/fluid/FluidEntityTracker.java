...
public class FluidEntityTracker
{
    ...
    private Map<Long, List<ItemStack>> items;
    ...
    {
        this.items.computeIfAbsent(pos.toLong(), k -> new LinkedList<>()).add(item.getItem());
    }
    
    public static List<ItemStack> getItems(World world, BlockPos pos)
    {
        return FluidEntityTracker.get(world).getItems(pos);
    }
    
    public List<ItemStack> getItems(BlockPos pos)
    {
    ...
    public static void forEach(World world, BiConsumer<Long, List<ItemStack>> action)
    {
        FluidEntityTracker.get(world).forEach(action);
    }
    
    public void forEach(BiConsumer<Long, List<ItemStack>> action)
    {
